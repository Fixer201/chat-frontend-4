...
function generateMockGroupParticipants(chat: ApiChatItem, seed: number): GroupParticipant[] {
  const firstNames = ['Алексей','Мария','Сергей','Екатерина','Дмитрий','Ольга','Иван','Анна','Михаил','Наталья']
  const lastNames = ['Петров','Иванова','Смирнов','Кузнецова','Федоров','Николаева','Воробьев','Павлова','Козлов','Орлова']
  ...
  for (let i = 1; i < count; i++) {
    ...
    participants.push({
      ...
      wasOnlineAt: Date.now() - Math.floor(Math.random() * 3600000),
    ...
    return Array(count)
        ...
        .map((_, index) => {
            ...
            if (chatType.includes('group')) {
                const participants = generateMockGroupParticipants(chatItem, index)
                saveGroupParticipants(chatItem.chat_key, participants)
            }
