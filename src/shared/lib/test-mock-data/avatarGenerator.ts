...
import { AVATAR_SOURCES, DEFAULT_AVATAR_SOURCE } from './avatarSources';
...
export const generateAvatarUrl = (
  ...
  switch (source) {
    ...
    case AVATAR_SOURCES.FLICKR:
      const categories = ['person', 'portrait', 'face', 'people'];
      const category = categories[seedStr.length % categories.length];
    ...
    case AVATAR_SOURCES.PLACEHOLDER:
      ...
      const color = colors[seedStr.length % colors.length];
 ...
 */
export const checkIfSourceWorks = async (source: string): Promise<boolean> => {
  try {
    const testUrl = generateAvatarUrl('test', 10, 10, source);
...
/**
 * Показывает все источники и их статус
 */
export const showAllSourcesStatus = async (): Promise<void> => {
  console.log('Проверяем доступность источников аватарок:');
  
  for (const [key, source] of Object.entries(AVATAR_SOURCES)) {
    const isWorking = await checkIfSourceWorks(source);
    console.log(`${key}: ${source} - ${isWorking ? '✓ Работает' : '✗ Не работает'}`);
  }
};