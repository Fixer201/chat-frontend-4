...
export function loadContactsFromStorage():
    ...
    try {
        ...
        console.error(
            'Ошибка при чтении контактов из localStorage:',
            error,
        )
    }
    return null
...
export function saveContactsToStorage(
    ...
    try {
        ...
        console.error(
            'Ошибка при сохранении контактов в localStorage:',
            error,
        )
    }
}
 ...
 */
export function addContactToStorage(
    newContact: Contact,
): void {
    const existing = loadContactsFromStorage() || []
    const exists = existing.some(
        (c) => c.uid === newContact.uid,
    )
    if (!exists) {
 ...
 */
export function removeContactsFromStorage(
    uids: string[],
): void {
    const existing = loadContactsFromStorage() || []
    const filtered = existing.filter(
        (c) => !uids.includes(c.uid),
    )
