...
export const backendFileToBaseFile = (
...
export const mockFileToBaseFile = (
    mockFile: MockFile,
    id: number,
): BaseFile => {
    ...
    const randomSize =
        mockFile.size || Math.random() * 100 * 1024 * 1024 // до 100 МБ
    ...
    const randomDate =
        mockFile.created_at ||
        Math.floor(
            Math.random() * (Date.now() - threeMonthsAgo),
        ) + threeMonthsAgo
...
export const transformFiles = (
    ...
    return files.map((file, index) => {
        if (source === 'backend') {
            return backendFileToBaseFile(
                file as BackendFile,
            )
        } else {
            return mockFileToBaseFile(
                file as MockFile,
                index + 1,
            )
        }
