...
export const generateRandomSender = (
    ...
    const lastName =
        lastNames[(index + 1) % lastNames.length]

    return `${firstName} ${lastName}`
}

// Функция получения инициалов отправителя
export const getSenderInitials = (
    sender: string,
): string => {
...
export const backendLinkToBaseLink = (
    ...
    return {
        ...
        sender,
        date: formatFileDate(
            backendLink.created_at ||
                backendLink.updated_at,
        ),
...
export const mockLinkToBaseLink = (
    mockLink: MockLink,
    index: number,
): BaseLink => {
    const id = index + 1
    ...
    const title =
        mockLink.title ||
        (usePredefined
            ? PREDEFINED_TITLES[index]
            : getTitleFromUrl(mockLink.url))
    const sender =
        mockLink.sender ||
        (usePredefined
            ? PREDEFINED_SENDERS[index]
            : generateRandomSender(index))
    const date =
        mockLink.date ||
        (usePredefined
            ? PREDEFINED_DATES[index]
            : formatFileDate(
                  mockLink.createdAt || Date.now(),
              ))
...
export const transformLinks = (
    ...
    return links.map((link, index) => {
        if (source === 'backend') {
            return backendLinkToBaseLink(
                link as BackendLink,
                index + 1,
            )
        } else {
            return mockLinkToBaseLink(
                link as MockLink,
                index,
            )
        }
