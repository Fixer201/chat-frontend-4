...
>(function CustomScrollbar(
    ...
    const updateThumbOnly = useCallback(() => {
        ...
        const thumbTop = (scrollTop / maxScroll) * (containerHeight - finalThumbHeight)

        thumb.style.height = `${finalThumbHeight}px`
        thumb.style.top = `${thumbTop}px`
        container.classList.remove(customStyle['custom-scrollbar-hidden'])
    }, [])
    ...
    const handleWheel = useCallback(
        (e: WheelEvent) => {
            ...
            if (process.env.NODE_ENV !== 'production') {
                console.debug('[CustomScrollbar] wheel', { deltaY: e.deltaY, scrollTopBefore: content.scrollTop, maxScroll })
            }
            ...
            if (maxScroll <= 0) {
                ...
                if (content.scrollTop <= 1 && e.deltaY < 0) {
                    if (process.env.NODE_ENV !== 'production') {
                        console.debug('[CustomScrollbar] attempt beyond top (not scrollable)', { deltaY: e.deltaY })
                    }
            ...
            if (content.scrollTop <= 1 && e.deltaY < 0) {
                if (process.env.NODE_ENV !== 'production') {
                    console.debug('[CustomScrollbar] attempt beyond top', { deltaY: e.deltaY })
                }
                if (onAttemptScrollBeyondTop) {
                    onAttemptScrollBeyondTop(e.deltaY, 0)
                }
    ...
    useEffect(() => {
        ...
        const touchStartHandler = (e: TouchEvent) => {
            startTouchYRef.current = e.touches[0]?.clientY || 0
        }
        ...
        const touchMoveHandler = (e: TouchEvent) => {
            ...
            if (process.env.NODE_ENV !== 'production') {
                console.debug('[CustomScrollbar] touchmove', { deltaY, scrollTopBefore: content.scrollTop })
            }
            ...
            if (maxScroll <= 0) {
                if (content.scrollTop <= 1 && deltaY < -10) {
                    if (process.env.NODE_ENV !== 'production') {
                        console.debug('[CustomScrollbar] attempt beyond top touch (not scrollable)', { deltaY })
                    }
                    if (onAttemptScrollBeyondTop) {
            ...
            if (content.scrollTop <= 1 && deltaY < -10) {
                if (process.env.NODE_ENV !== 'production') {
                    console.debug('[CustomScrollbar] attempt beyond top touch', { deltaY })
                }
                if (onAttemptScrollBeyondTop) {
                    onAttemptScrollBeyondTop(deltaY, 0)
                }
    ...
    useImperativeHandle(ref, () => ({
        ...
        animateScrollTo: (target: number, duration = 220) => {
            const content = contentRef.current
            if (!content) return
            ...
            const step = (now: number) => {
                ...
                const current = start + (dest - start) * eased
        ...
        scrollToBottom: function () {
            const content = contentRef.current
            if (!content) return
            const dest = content.scrollHeight - content.clientHeight
            // @ts-expect-error - вызов animateScrollTo из объекта
