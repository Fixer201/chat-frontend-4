...
export function CustomScrollbar({ children, className = '' }) {
  ...
  const updateThumbPosition = useCallback(() => {
    ...
    const minHeight = 20
    const finalThumbHeight = Math.max(thumbHeight, minHeight)
    
    // Позиция ползунка
    const maxScroll = scrollHeight - clientHeight
    const thumbTop = maxScroll > 0 
      ? (scrollTop / maxScroll) * (containerHeight - finalThumbHeight)
      : 0
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    const containerHeight = container.clientHeight
    const thumbHeight = thumbRef.current?.clientHeight || 20
    const trackHeight = containerHeight - thumbHeight
    
    // Рассчитываем новый scrollTop
    const scrollRatio = content.scrollHeight / containerHeight
    const newScrollTop = startScrollTopRef.current + deltaY * scrollRatio
    
    // Ограничиваем значения
    content.scrollTop = Math.max(
      0,
      Math.min(newScrollTop, content.scrollHeight - content.clientHeight)
    )
  }, [])
  ...
  }, [handleMouseMove, handleMouseUp, handleWheel, updateThumbPosition])

  return (
    <div 
      ref={containerRef}
      className={`${customStyle['custom-scroll-container']} ${className}`}
    >
      <div 
        ref={contentRef}
        className={`${customStyle['custom-scroll-content']}`}
      >
        {children}
      </div>
      <div className={`${customStyle['custom-scrollbar']}`}>
        <div className={`${customStyle['custom-scrollbar-track']}`}/>
        <div 
          ref={thumbRef}
          className={`${customStyle['custom-scrollbar-thumb']}`}
