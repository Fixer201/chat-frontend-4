...
import { initializeContacts, saveContactsToStorage } from '@shared/lib/localStorageContacts'
  ...
  reducers: {
    ...
    removeContacts: (state, action: PayloadAction<string[]>) => {
      state.list = state.list.filter(
        (contact) => !action.payload.includes(contact.uid)
      )
...
export const { setContacts, removeContacts } = contactsTemp.actions
