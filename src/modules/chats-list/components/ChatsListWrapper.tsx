...
export default function ChatsListWrapper() {
  const { createGroup, createChannel, loadChats } = useChats()

  const [currentView, setCurrentView] = useState<View>('chats')
  const [groupData, setGroupData] = useState<onNextProps | null>(null)
  const [channelData, setChannelData] = useState<onNextProps | null>(null)
  const [selectedContacts, setSelectedContacts] = useState<Contact[]>([])
  const [isCreating, setIsCreating] = useState(false)
  const [createError, setCreateError] = useState<string | null>(null)
  const [successMessage, setSuccessMessage] = useState<string | null>(null) // для уведомления
  ...
  const handleCreateGroup = () => setCurrentView('create-group')
  const handleCreateChannel = () => setCurrentView('create-channel')
  ...
  const handleBackFromGroupMembers = () => setCurrentView('create-group')
  const handleBackFromChannelMembers = () => setCurrentView('create-channel')

  const handleNextFromCreateGroup = (payload: onNextProps | string) => {
  ...
  const handleNextFromCreateChannel = (payload: string | onNextProps) => {
    if (typeof payload === 'object') {
  ...
  const handleFinishGroupCreation = async (contacts: Contact[]) => {
    if (!groupData) return
    ...
    try {
      ...
      setSuccessMessage(`Группа "${groupData.name}" успешно создана!`)
      setCurrentView('chats')
      setGroupData(null)
      setSelectedContacts([])
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : 'Неизвестная ошибка при создании группы'
  ...
  const handleFinishChannelCreation = async (contacts: Contact[]) => {
    ...
    try {
      await createChannel(channelData, contacts).unwrap()
      setSuccessMessage(`Канал "${channelData.name}" успешно создан!`)
      setCurrentView('chats')
      setChannelData(null)
      setSelectedContacts([])
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : 'Неизвестная ошибка при создании канала'
      setCreateError(errorMessage)
    } finally {
      setIsCreating(false)
    }
  ...
  const renderView = () => {
    switch (currentView) {
      case 'chats':
        return <ChatsList onCreateGroup={handleCreateGroup} onCreateChannel={handleCreateChannel} />
      ...
      case 'group-members':
        return groupData ? (
          ...
          <div className="p-4">
            ...
            <button onClick={() => setCurrentView('chats')} className={`
      ...
      case 'channel-members':
        return channelData ? (
          ...
          <div className="p-4">
            <p>Ошибка: название канала не найдено</p>
            <button onClick={() => setCurrentView('chats')} className={`
              mt-4 rounded bg-gray-main px-4 py-2
            `}>
              Вернуться к чатам
      ...
      {successMessage && (
        ...
        `}>
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleCloseToast} className={`
              ml-2 text-white
              hover:text-gray-200
            `}>
              ×
