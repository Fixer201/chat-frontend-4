...
export default function ChatInfoSidebar() {
  ...
  const { chatSettings, toggleNotifications, deleteChat, selectChat } = useChats()
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      try {
        const rawData = getChatByIdFromStorage(selectedChatId)
        if (rawData) {
          const transformed = transformFromApi<ApiChatItem>(rawData) as ChatItem
          setChatData(transformed)
        } else {
  ...
  const handleClearChat = useCallback((deleteForEveryone: boolean) => {
 ...
 const handleCloseSidebar = useCallback(() => {
  ...
  const { chatType } = chatData
   ...
   if (chatType.includes('group')) {
    const participantsCount = chatData.participants?.length || 0
    const inviteLink = chatData.chatKey ? `https://a-chat.su/invite/${chatData.chatKey}` : undefined
    const currentNotifications = selectedChatId ? chatSettings[selectedChatId]?.notificationsEnabled ?? true : true
