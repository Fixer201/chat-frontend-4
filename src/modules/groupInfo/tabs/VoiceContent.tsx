...
import { transformFiles, formatAudioDuration } from '@shared/lib/fileUtils'
import type { MockFile, AudioFile } from '@shared/types/file'
import { loadGroupAudio, initGroupAudio } from '@shared/lib/localStorageGroupAudio'
...
export default function VoiceContent({ chatUid }: VoiceContentProps) {
  const [visible, setVisible] = useState(false)
  const [loading, setLoading] = useState(true)
  const [audioMessages, setAudioMessages] = useState<AudioFile[]>([])
  ...
  const loadAudio = async () => {
    ...
    try {
      ...
      if (!audioData) {
        audioData = initGroupAudio(chatUid, DEFAULT_AUDIO_FILES)
      }
      const transformedFiles = transformFiles(audioData.results, 'mock')
      const audioFiles = transformedFiles.filter(file => file.type === 'audio')
      
      const audioMessagesData: AudioFile[] = audioFiles.map(file => {
        const randomDuration = Math.floor(Math.random() * (240 - 30) + 30)
        return {
          ...file,
          duration: formatAudioDuration(randomDuration),
          isPlaying: false,
          currentTime: 0,
    ...
    if (!audioRefs.current[id]) {
      ...
      audio.addEventListener('ended', () => {
        setAudioMessages(prev => 
          prev.map(msg => 
            msg.id === id 
              ? { ...msg, isPlaying: false, currentTime: 0 } 
              : msg
          )
        )
  ...
  if (loading) {
    return (
      <div className="flex h-64 items-center justify-center">
        <div className="text-text-gray">Загрузка аудио...</div>
      </div>
    )
  }
    ...
    `}>
      <div className="space-y-0">
        {audioMessages.map((message) => {
          const remainingTime = message.totalDuration - message.currentTime
          const displayTime = message.isPlaying 
            ? formatAudioDuration(remainingTime)
            : message.duration
          
          const progress = message.totalDuration > 0
            ? (message.currentTime / message.totalDuration) * 100
            : 0
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <button 
                  onClick={() => togglePlay(message.id, message.url)}
                  className={`
