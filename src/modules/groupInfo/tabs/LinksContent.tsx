...
import { loadGroupLinks, initGroupLinks } from '@shared/lib/localStorageGroupLinks'
...
export default function LinksContent({ chatUid }: LinksContentProps) {
  const [visible, setVisible] = useState(false)
  const [linksState, setLinksState] = useState<BaseLink[]>([])
  ...
  const loadLinks = async () => {
    ...
    try {
      ...
      if (!linksData) {
        linksData = initGroupLinks(chatUid, DEFAULT_MOCK_LINKS)
      }
      const transformedLinks = transformLinks(linksData.results, 'mock')
      setLinksState(transformedLinks)
    } catch (error) {
      console.error('Ошибка загрузки ссылок:', error)
      const transformedLinks = transformLinks(DEFAULT_MOCK_LINKS, 'mock')
      setLinksState(transformedLinks)
    } finally {
      setLoading(false)
    }
  ...
  if (loading) {
    return (
      <div className="flex h-64 items-center justify-center">
        <div className="text-text-gray">Загрузка ссылок...</div>
      </div>
    )
  }
        ...
        {linksState.map((link) => (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              `}>
                {link.senderInitials || link.sender.charAt(0).toUpperCase()}
              </div>
              <div className="min-w-0 flex-1 flex-col justify-between">
                ...
                <div className="flex items-center text-sm text-text-gray">
                  <span className="truncate">{link.sender}</span>
