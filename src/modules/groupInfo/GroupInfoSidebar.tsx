...
function getTabContent(tabId: TabId, chatKey: string, chatUid: string, setDynamicTabTitle: (t: string | null) => void) {
    switch (tabId) {
        case 'participants':
            return <ParticipantsContent chatKey={chatKey} onTitleChange={setDynamicTabTitle} />
        case 'media':
...
export default function GroupInfoSidebar({
    ...
    const {
        activeTab, viewMode, isTransitioning, isMouseOver, setIsMouseOver,
        dynamicTabTitle, setDynamicTabTitle, hideTabScrollbarDuringReturn,
        tabScrollPositions, tabsRef, containerRef, mainContentRef, tabsContainerRef,
        tabs, handleMainTabClick, handleBackFromTab, handleMainTouchStart,
        handleMainTouchMove, handleMainWheel, handleMainScroll,
        handleTabScrollEvent, handleTabContentTabClick, handleAttemptReturn, getTabTitle,
    } = useGroupInfoSidebar()
  ...
  const handleClearChatConfirm = useCallback(async (deleteForEveryone: boolean) => {
    ...
    if (viewMode === 'tab') {
        return (
            <TabLayout
                ...
                initialScrollTop={tabScrollPositions[activeTab]}
            >
                {getTabContent(activeTab, chatKey, chatUid, setDynamicTabTitle)}
            </TabLayout>
        )
    }
        ...
        >
            ...
            `}>
                <Button
                    ...
                    `}
                >
                    <Image src="/icons/detailInfo/detailInfoClose.svg" alt="Закрыть" width={24} height={24} />
                </Button>
                ...
                <div className="flex items-center gap-3">
                    <Button
                        onClick={() => console.log('Дополнительные настройки')}
                        ...
                        `}
                    >
                        <Image src="/icons/detailInfo/detailInfoSettings.svg" alt="Детальные настройки" width={24} height={24} />
                    </Button>

                    <DropdownMenuButton
                        triggerIcon={
                            <Image src="/icons/detailInfo/detailInfoDropdown.svg" alt="Настройки" width={24} height={24} className={`
                        ...
                        items={[
                            {
                                label: 'Очистить чат',
                                icon: <Image src="/icons/clean.svg" alt="" width={24} height={24} />,
                                onClick: () => setClearChatModalOpen(true),
                            },
                            {
                                label: 'Покинуть группу',
                                icon: <Image src="/icons/leave.svg" alt="" width={24} height={24} />,
                                onClick: () => setLeaveGroupModalOpen(true),
                                hasDivider: true,
                            },
                            {
                                label: 'Удалить группу',
                                icon: <Image src="/icons/delete.svg" alt="" width={24} height={24} />,
            ...
            <div
                ref={mainContentRef}
                className={cn('scrollbar-hide flex-1 overflow-auto', `
            ...
            >
                ...
                <div className="relative">
                    <div className="relative h-60 w-full overflow-hidden">
                        <Image src="/images/tempSIdebarInfo.png" alt="Группа" fill className={`
                          object-cover
                        `} />
                    </div>
                    <div className={`
                    ...
                    `}>
                        ...
                        <p className="mt-1 text-lg text-white/90">
                            {participantsCount} {getNoun(participantsCount, 'участник', 'участника', 'участников')}
                        </p>
                    </div>
                </div>
                ...
                <div className="bg-gray-50 px-4 py-3">
                    ...
                    <div className="flex items-center justify-between">
                        <span className="text-base font-medium text-text-black">Уведомление</span>
                        <button
                            ...
                            className={cn(
                                `
                                  ...
                                  focus:outline-none
                                `,
                                notificationsEnabled ? 'bg-blue-500' : `
                                  bg-gray-300
                                `,
                            )}
                        >
                    ...
                    {inviteLink && (
                        <div className="mx-0 my-1 rounded-md bg-white-bg p-1">
                            <div className="flex flex-col justify-between p-0.5">
                                ...
                                `}>
                                    Ссылка на приглашение в группу
                                </span>
                                <div className={`
                    ...
                    {/* Табы */}
                    <div ref={tabsContainerRef} className="mt-1">
                            ...
                            `}>
                                {tabs.map((tab, index) => (
                                    <button
                                        ...
                                        onClick={() => handleMainTabClick(tab.id, index)}
                                        className={cn(
                                            `
                    ...
                    `}>
                        <TabContentPreview chatKey={chatKey} activeTab={activeTab} />
                    </div>
                </div>
            </div>
