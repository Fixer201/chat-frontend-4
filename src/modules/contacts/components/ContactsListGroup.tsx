...
import { removeContacts, setContacts } from '@redux/slices/contactsSliceTemp'
...
export default memo(function ContactsListGroup({ 
    ...
    const [participantToDelete, setParticipantToDelete] = useState<GroupParticipant | null>(null)
    ...
    const { filteredValue: filteredParticipants } = useSearch(
        participants,
        searchValue,
        [
            (contact) =>
    ...
    const handleDeleteParticipant = (participant: GroupParticipant) => {
    ...
    const handleConfirmDeleteParticipant = () => {
        ...
        removeGroupParticipant(chatKey, participantToDelete.uid)
    ...
    return (
        <>
            <div className="mt-2 flex h-1/12 min-h-15 items-center px-4">
                ...
                >
                    <Image src="/icons/addToGroup.svg" alt="Добавить" width={16} height={16} />
                    <p className={`
                    ...
                    {owner && (
                        <>
                            ...
                            <ContactItemGroup
                                ...
                                onSetSelectedContact={(uid: string) =>
                    ...
                    {filteredParticipants && filteredParticipants.length > 0 ? (
                            filteredParticipants.map((contact) => (
                                <ContactItemGroup
                                    ...
                                    onDelete={(contact) => handleDeleteParticipant(contact as GroupParticipant)}
                                />
                            ))
                        ) : filteredParticipants.length === 0 && searchValue.trim() ? (
                        <div className={`
