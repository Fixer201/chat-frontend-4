...
@theme inline {
    --color-accent-violet-primary: var(--app-accent-violet-primary);
    --color-accent-violet-dark: var(--app-accent-violet-dark);
}
```
...
<div className="form-field">
    <label htmlFor="email" className="text-sm font-medium text-text-black">
        Email
    </label>
    ...
    <input
        ...
        className={clsx(
            ...
            hasError && 'border-system-red bg-system-red-surface text-system-red',

            // Focus ring
            'focus:outline-none focus:ring-2 focus:ring-offset-1',
            hasError ? 'focus:ring-system-red/30' : 'focus:ring-accent-violet-primary/30'
        )}
    />

    {hasError && (
        <p id="error-msg" className="text-xs text-system-red mt-1">
            Некорректный email
        </p>
    )}
</div>
    ...
    return (
        <button
            className={twMerge(
                ...
                color === 'primary' && 'bg-accent-violet-primary text-white-bg',
                color === 'error' && 'bg-system-red text-white-bg',
...
<div className={isDark ? 'bg-accent-violet-primary' : 'bg-white-bg'}>
    ...
</div>
```
...
```jsx
className = "bg-white-bg dark:bg-gray-main text-text-black dark:text-gray-50"
```
